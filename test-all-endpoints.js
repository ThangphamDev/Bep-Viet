...
async function runTest(name, testFn) {
  try {
    ...
    if (result.success) {
      ...
      console.log(`❌ FAILED: ${name} - ${result.error || 'Status: ' + result.status}`);
      results.failed++;
    }
...
async function testComments() {
  const response = await makeRequest(`${BASE_URL}/comments/recipes/1`);
  return {
    success: response.status === 200 && response.data.success,
    status: response.status,
    data: response.data
...
async function testRatings() {
  const response = await makeRequest(`${BASE_URL}/ratings/recipes/1`);
  return {
    success: response.status === 200 && response.data.success,
    status: response.status,
    data: response.data
...
// Protected endpoints tests (require authentication)
...
async function runAllTests() {
  console.log('🚀 Starting Bếp Việt API Tests');
  ...
  await runTest('GET /suggestions', testSuggestions);
  await runTest('GET /meal-plans', testMealPlans);
  await runTest('GET /pantry', testPantry);
  await runTest('GET /shopping', testShopping);
  await runTest('GET /community', testCommunity);
  await runTest('GET /comments', testComments);
  await runTest('GET /ratings', testRatings);
  await runTest('GET /family', testFamily);
  await runTest('GET /advisory', testAdvisory);
  await runTest('GET /analytics', testAnalytics);
  await runTest('GET /moderation', testModeration);
